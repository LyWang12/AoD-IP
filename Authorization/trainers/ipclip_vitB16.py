...
class CustomCLIP(nn.Module):
    ...
    @autocast()
    def forward(self, s_image, e_image=None, domain=None, mode=None):
        if mode == "testing":
            image = s_image
            ...
            return logits_source_with_author_scr, logits_expend_with_author_scr, source_text_features_with_author_scr, expend_text_features_with_author_scr
...
class IPCLIPB16(TrainerXU):
    ...
    def build_model(self):
        ...
        if self.cfg.TRAIN.COUNT_ITER == "train_x":
            ...
            self.num_batches = min(len_train_loader_x, len_train_loader_e)
        else:
            raise ValueError
    ...
    def run_epoch(self):
        ...
        for self.batch_idx in range(self.num_batches):
            ...
            loss_summary = self.forward_backward(batch_x, batch_e)
    ...
    def forward_backward(self, batch_s, batch_e):
        self.entropy = entropy_loss()
        kl_loss = nn.KLDivLoss(reduction="batchmean")
        image_s, label_s, image_e, label_e = self.parse_batch_train(batch_s, batch_e)

        prec = self.cfg.TRAINER.IPCLIPB16.PREC
        if prec == "amp":
            with autocast():
                logits_source_with_author_scr, logits_expend_with_author_scr, source_text_features_with_author_scr, expend_text_features_with_author_scr = self.model(image_s, image_e, mode='training')
                ...
                loss = loss_ce_s_with_a_scr - 0.1 * loss_ce_s_with_a_scr_u + 0.1 * loss_ce_e_with_a_scr - loss_kl_ae
    ...
    def parse_batch_train(self, batch_s, batch_e):
        input_s = batch_s["img"]
        ...
        return input_s, label_s, input_e, label_e
    ...
    def test(self, domain_scr):
        ...
        test_loaders = {
            "art_water": self.test_loader_x,
            "art": self.add_test_loader_1,
            "clipart": self.add_test_loader_2,
            "product": self.add_test_loader_3,
            "realworld": self.add_test_loader_4,
        }
